...
def classify_smri_data(features_csv, phenotype_csv):
    """
    Performs classification on sMRI features using multiple models and a majority voting ensemble.

    Args:
        features_csv (str): Path to the sMRI features CSV.
        phenotype_csv (str): Path to the phenotype CSV to get the labels.
    """
    ...
    # Merge to ensure correct labels and alignment
    ...
    plt.title(f'AUC-ROC Curves for sMRI ({os.path.basename(features_csv)})')
    plt.legend(loc='lower right')
    plt.savefig(f'roc_curve_smri_{os.path.basename(features_csv).split(".")[0]}.png')
    plt.show()